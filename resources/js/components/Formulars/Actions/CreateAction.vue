            ...
            <div class="modal-body">
                <form role="form" class="form-horizontal">
                    ...
                    <div class="form-group d-flex justify-content-between px-3">
                        ...
                        <div style="width: 51%">
                            <input autocomplete="total"  class="form-control" :class="invalidsNewAction.total !== undefined ? 'is-invalid' : '' " v-model="newAction.total" placeholder="Le nom de l'action" type="text">
                            <i class="m-0 p-0 mt-1 text-danger" v-if="invalidsNewAction.total !== undefined">{{ invalidsNewAction.total[0] }}</i>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="col-sm-12">
                            <textarea class="form-control" :class="invalidsNewAction.description !== undefined ? 'is-invalid' : '' " v-model="newAction.description" placeholder="Décrivez">
    ...
    export default {
        ...
        methods :{
           ...
           createAction(){
                ...
                this.$store.dispatch('createAction', {action: this.theAction, token: this.token})
           },
           cancelImage(){
        ...
        computed: mapState([
            'user', 'member', 'active_member', 'connected', 'invalidsNewAction', 'newAction', 'token'
        ])
    }
</script>
